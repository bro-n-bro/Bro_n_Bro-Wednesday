...
from db import generate_tx, get_winners, write_winners_to_db, get_rewards, distribute_rewards
...
def main():
    ...
    tx = generate_tx(PRIZE_POOL_ADDRESS_MNEMONIC, 1_000_000, memo, PRIZE_POOL_ADDRESS_NUMBER)
    print('the resulting tx:')
    for index, row in distribution_df.iterrows():
        tx.add_transfer(row['address'], row['payout'], row['denom'])
        print(row['address'], row['payout'], row['denom'])
